...
if __name__ == "__main__":
    variant = dict(
        ...
        env_name=ENV_NAME,
        env_kwargs=dict(),
        do_offline_training=True,  # here we specify we want to do offline training
        do_online_training=False,
        teacher_data_files=['d4rl-hopper-medium-expert'],  # download this with example script
        ...
        trainer_kwargs=dict(
            ...
            real_data_pct=0.05,
            sampling_mode='var_disagreement',  # Here we specify the MOPO algorithm (special case of MBPO)
            sampling_kwargs=dict(
                reward_penalty=1,  # Original paper searched this out of (1, 5)
            ),
            policy_kwargs=dict(
                discount=0.99,
    ...
    sweep_values = {
        'trainer_kwargs/sampling_kwargs/reward_penalty': [1, 5],  # grid search over hyperparameter
    }
